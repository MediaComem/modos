...
export class initPlosCat1603889820064 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`INSERT INTO modos.ploscategories (category) VALUES ('obstacle'), ('step'), ('coating'), ('security'), ('slope'), ('width'), ('other-negative'), ('positive') ON CONFLICT (id) DO NOTHING;`, undefined);
        await queryRunner.query(`CREATE TABLE IF NOT EXISTS modos.impacts (id INTEGER GENERATED ALWAYS AS IDENTITY, impact_name TEXT NOT NULL, CONSTRAINT impact_pkey PRIMARY KEY (id), CONSTRAINT check_impact_name CHECK (impact_name='low' OR impact_name='mild' OR impact_name='moderate' OR impact_name='high' OR impact_name='critical'));`, undefined);
        await queryRunner.query(`INSERT INTO modos.impacts (impact_name) VALUES ('low'), ('mild'), ('moderate'), ('high'), ('critical') ON CONFLICT (id) DO NOTHING;`, undefined);
        await queryRunner.query(`CREATE TABLE IF NOT EXISTS modos.descriptions (id INTEGER GENERATED ALWAYS AS IDENTITY, ploscategory_id INT REFERENCES modos.ploscategories(id) NOT NULL, impact_id INT REFERENCES modos.impacts(id) NOT NULL, "freetext" CHARACTER VARYING(4096), CONSTRAINT descriptions_pkey PRIMARY KEY (id));`, undefined);
    }

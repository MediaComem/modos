...
class BinaryModel(BaseModel):
    def __init__(self, config, pretrained_model=MobileNetV2, preprocess_input=preprocess_input):
        super(BinaryModel, self).__init__(config)
        ...
        with strategy.scope():
            ...
            self.metrics = [
                ...
                F1Score(num_classes=2, name='f1-score', average='weighted')
            ]
    ...
    #     x = Flatten()(x)x = Dropout(0.2)(x)
        x = Dense(64, kernel_regularizer=regularizers.l2(
            0.001), activation='relu')(x)
        x = Dropout(0.3)(x)
        x = Dense(32, kernel_regularizer=regularizers.l2(
            0.001), activation='relu')(x)
        x = Dropout(0.4)(x)
        x = Dense(16, kernel_regularizer=regularizers.l2(
            0.001), activation='relu')(x)
        outputs = Dense(2, activation="softmax")(x)  # prediction layer

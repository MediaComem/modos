...
function getUserById(req, res, next) {
  const userModel = models.sequelize.model('user');
  userModel.findByPk(req.params.userId, { rejectOnEmpty: true })
    .then(function (user) {
...
function updateUser(req, res, next) {
  const userModel = models.sequelize.model('user');
  userModel.update(req.body, { where: { id: req.params.userId } })
    .then(function () {
      res.status(204).send();
    })
    .catch(function (err) {
...
function getEventsFromUser(req, res, next) {
  const userEventModel = models.sequelize.model('userEvent');
  const eventModel = models.sequelize.model('event');

  userEventModel.findAll({ where: { userId: req.params.userId }, attributes: ['eventId'] })
...
function addUserToEvent(req, res, next) {
  const userEventModel = models.sequelize.model('userEvent');
  userEventModel.create(req.params)
...
function getObservationsFromUser(req, res, next) {
  const observationModel = models.sequelize.model('observation');
  observationModel.findAll({ where: { userId: req.params.userId } })
...
router.get('/:userId/events', getEventsFromUser);
router.get('/:userId/observations', getObservationsFromUser);
router.post('/:userId/join/:eventId', addUserToEvent);
